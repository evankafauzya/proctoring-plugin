 ...
 * @param int $usercount Number of users detected
 * @param string $severity Alert severity level (critical, warning, info)
 * @param string|null $screenshot Screenshot data (base64)
 * @return void
 */
function quizaccess_proctoring_log_multiuser_alert($reportid, $courseid, $quizid, $userid, $usercount, $severity = 'warning', $screenshot = null) {
    global $DB;
    ...
    $record->userid = $userid;
    $record->user_count = $usercount;
    $record->alert_severity = $severity;
    $record->screenshot = $screenshot;
    $record->timedetected = time();

    $DB->insert_record('quizaccess_proctoring_multiuser_alerts', $record);
    mtrace("Multi-user alert logged: {$usercount} users detected for user {$userid}");
}

/**
 * Log eye tracking/gaze alert
 *
 * @param int $reportid Report ID
 * @param int $courseid Course ID
 * @param int $quizid Quiz ID
 * @param int $userid User ID
 * @param int $eyeNotVisible Count of frames where eyes not visible
 * @param int $lookingAway Count of times looking away
 * @param string $severity Alert severity level
 * @return void
 */
function quizaccess_proctoring_log_eyetrack_alert($reportid, $courseid, $quizid, $userid, $eyeNotVisible = 0, $lookingAway = 0, $severity = 'warning') {
    global $DB;
    ...
    $record->quizid = $quizid;
    $record->userid = $userid;
    $record->eye_not_visible = $eyeNotVisible;
    $record->looking_away = $lookingAway;
    $record->alert_severity = $severity;
    $record->timedetected = time();

    $DB->insert_record('quizaccess_proctoring_eyetrack_alerts', $record);
    mtrace("Eye tracking alert logged for user {$userid}: Eyes not visible {$eyeNotVisible}x, Looking away {$lookingAway}x");
}

/**
 * Log periodic re-verification (every 40 seconds)
 *
 * @param int $reportid Report ID
 * @param int $courseid Course ID
 * @param int $quizid Quiz ID
 * @param int $userid User ID
 * @param int $reverificationNumber Sequential count of re-verifications
 * @param string $status Status of verification (pending, verified, failed)
 * @param int $faceMatchScore Face match score for this verification
 * @param int $pausedDuration Duration quiz was paused
 * @param string|null $screenshot Screenshot data
 * @return int Record ID
 */
function quizaccess_proctoring_log_reverification($reportid, $courseid, $quizid, $userid, $reverificationNumber, $status = 'pending', $faceMatchScore = 0, $pausedDuration = 0, $screenshot = null) {
    global $DB;
    ...
    $record->quizid = $quizid;
    $record->userid = $userid;
    $record->reverification_number = $reverificationNumber;
    $record->verification_status = $status;
    $record->face_match_score = $faceMatchScore;
    $record->quiz_paused_duration = $pausedDuration;
    $record->screenshot = $screenshot;
    $record->timecreated = time();
    $record->timecompleted = ($status !== 'pending') ? time() : 0;

    $id = $DB->insert_record('quizaccess_proctoring_reverification', $record);
    mtrace("Re-verification logged for user {$userid} (#{$reverificationNumber}): Status {$status}, Score {$faceMatchScore}");

    return $id;
}

/**
 * Update re-verification status after verification is complete
 *
 * @param int $recordId Record ID from re-verification table
 * @param string $status New status (verified, failed)
 * @param int $faceMatchScore Face match score
 * @param int $pausedDuration Duration paused
 * @return void
 */
function quizaccess_proctoring_update_reverification($recordId, $status = 'verified', $faceMatchScore = 0, $pausedDuration = 0) {
    global $DB;
    ...
    $DB->update_record('quizaccess_proctoring_reverification', $record);
    mtrace("Re-verification updated: ID {$recordId}, Status {$status}");
}

/**
 * Generate comprehensive analytics report after quiz completion
 *
 * @param int $reportid Report ID
 * @param int $courseid Course ID
 * @param int $quizid Quiz ID
 ...
 */
function quizaccess_proctoring_generate_analytics($reportid, $courseid, $quizid, $userid) {
    global $DB;

    // Count total captures
    $totalCaptures = $DB->count_records('quizaccess_proctoring_logs', [
        'userid' => $userid,
        'quizid' => $quizid,
        'courseid' => $courseid
    ]);

    // Count multi-user alerts
    $multiuserAlerts = $DB->count_records('quizaccess_proctoring_multiuser_alerts', [
        'reportid' => $reportid,
        'userid' => $userid
    ]);

    // Count eye tracking alerts
    $eyetrackAlerts = $DB->count_records('quizaccess_proctoring_eyetrack_alerts', [
        'reportid' => $reportid,
        'userid' => $userid
    ]);

    // Get re-verification data
    $reverifications = $DB->get_records('quizaccess_proctoring_reverification', [
        'reportid' => $reportid,
        'userid' => $userid
    ]);

    $reverificationsTriggered = count($reverifications);
    $reverificationsPassed = 0;
    $reverificationsFailed = 0;
    $totalPauseTime = 0;
    $totalFaceScores = 0;

    foreach ($reverifications as $rev) {
        if ($rev->verification_status === 'verified') {
            $reverificationsPassed++;
        } elseif ($rev->verification_status === 'failed') {
            $reverificationsFailed++;
        }
        $totalPauseTime += $rev->quiz_paused_duration;
        $totalFaceScores += $rev->face_match_score;
    }

    $averageFaceScore = ($reverificationsTriggered > 0) ? intval($totalFaceScores / $reverificationsTriggered) : 0;

    // Calculate risk score (0-100)
    $riskScore = quizaccess_proctoring_calculate_risk_score(
        $multiuserAlerts,
        $eyetrackAlerts,
        $reverificationsFailed,
        $averageFaceScore
    ...
    $record->quizid = $quizid;
    $record->userid = $userid;
    $record->total_captures = $totalCaptures;
    $record->multiuser_alerts = $multiuserAlerts;
    $record->eyetrack_alerts = $eyetrackAlerts;
    $record->reverifications_triggered = $reverificationsTriggered;
    $record->reverifications_passed = $reverificationsPassed;
    $record->reverifications_failed = $reverificationsFailed;
    $record->total_pause_time = $totalPauseTime;
    $record->average_face_match_score = $averageFaceScore;
    $record->risk_score = $riskScore;
    $record->risk_level = $riskLevel;
    $record->compliance_status = $complianceStatus;
    $record->timecreated = time();

    $analyticsId = $DB->insert_record('quizaccess_proctoring_analytics', $record);

    mtrace("Analytics generated for user {$userid}: Risk Score {$riskScore}, Risk Level {$riskLevel}, Compliance {$complianceStatus}");

    return $analyticsId;
}

/**
 * Calculate overall risk score based on proctoring alerts and verification data
 *
 * @param int $multiuserAlerts Number of multi-user alerts
 * @param int $eyetrackAlerts Number of eye tracking alerts
 * @param int $verificationsFailed Number of failed re-verifications
 * @param int $averageFaceScore Average face match score (0-100)
 * @return int Risk score (0-100)
 */
function quizaccess_proctoring_calculate_risk_score($multiuserAlerts, $eyetrackAlerts, $verificationsFailed, $averageFaceScore) {
    $riskScore = 0;

    // Multi-user alerts (max 40 points)
    $multiuserRisk = min($multiuserAlerts * 10, 40);
    $riskScore += $multiuserRisk;

    // Eye tracking alerts (max 30 points)
    $eyetrackRisk = min($eyetrackAlerts * 5, 30);
    $riskScore += $eyetrackRisk;

    // Failed verifications (max 20 points)
    $failureRisk = min($verificationsFailed * 10, 20);
    $riskScore += $failureRisk;

    // Face match score (if low, increase risk - max 10 points)
    if ($averageFaceScore < 50) {
        $riskScore += 10;
    } elseif ($averageFaceScore < 70) {
        $riskScore += 5;
    }
...
/**
 * Get analytics report for a quiz attempt
 *
 * @param int $reportid Report ID
 * @return stdClass|false Analytics record
 */
function quizaccess_proctoring_get_analytics($reportid) {
    global $DB;
    return $DB->get_record('quizaccess_proctoring_analytics', ['reportid' => $reportid]);
}

/**
 * Get all re-verification records for a report
 *
 * @param int $reportid Report ID
 * @return array Array of re-verification records
 */
function quizaccess_proctoring_get_reverifications($reportid) {
    global $DB;
    return $DB->get_records('quizaccess_proctoring_reverification', ['reportid' => $reportid], 'timecreated ASC');
}

/**
 * Get all multi-user alerts for a report
 *
 * @param int $reportid Report ID
 * @return array Array of multi-user alert records
 */
function quizaccess_proctoring_get_multiuser_alerts($reportid) {
    global $DB;
    return $DB->get_records('quizaccess_proctoring_multiuser_alerts', ['reportid' => $reportid], 'timedetected ASC');
}

/**
 * Get all eye tracking alerts for a report
 *
 * @param int $reportid Report ID
 * @return array Array of eye tracking alert records
 */
function quizaccess_proctoring_get_eyetrack_alerts($reportid) {
    global $DB;
    return $DB->get_records('quizaccess_proctoring_eyetrack_alerts', ['reportid' => $reportid], 'timedetected ASC');
}
