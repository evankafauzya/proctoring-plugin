...
class quizaccess_proctoring_external extends external_api {
    ...
    public static function validate_face($courseid, $cmid, $profileimage, $webcampicture, $parenttype, $faceimage, $facefound) {
        ...
        $result['warnings'] = $warnings;
        // Distinguish 'service unavailable' (backend unreachable / 5xx) from
        // 'invalid API key' so the student sees an accurate message.
        if ($currentdata->awsflag == 101) {
            $result['status'] = 'serviceunavailable';
        } else if ($currentdata->awsflag == 102) {
    ...
    /**
     * Log multi-user detection alert
     *
     * @return external_single_structure Alert response
     */
    public static function log_multiuser_alert_parameters() {
        return new external_function_parameters([
            'user_count' => new external_value(PARAM_INT, 'Number of users detected'),
            'severity' => new external_value(PARAM_ALPHA, 'Alert severity level', VALUE_DEFAULT, 'warning'),
            'timestamp' => new external_value(PARAM_INT, 'Alert timestamp', VALUE_DEFAULT, 0),
        ]);
    }

    public static function log_multiuser_alert($userCount, $severity = 'warning', $timestamp = 0) {
        global $DB, $USER;
        $params = self::validate_parameters(self::log_multiuser_alert_parameters(),
            ['user_count' => $userCount, 'severity' => $severity, 'timestamp' => $timestamp]);

        // Log the alert
        $record = new stdClass();
        $record->user_count = $params['user_count'];
        $record->alert_severity = $params['severity'];
        $record->timedetected = $params['timestamp'] ?: time();

        $id = $DB->insert_record('quizaccess_proctoring_multiuser_alerts', $record);

        return ['success' => true, 'id' => $id];
    }

    public static function log_multiuser_alert_returns() {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Operation success'),
            'id' => new external_value(PARAM_INT, 'Alert record ID'),
        ]);
    }
     ...
     * @return external_single_structure Alert response
     */
    public static function log_eyetrack_alert_parameters() {
        return new external_function_parameters([
            'eye_not_visible' => new external_value(PARAM_INT, 'Count of frames with no eyes', VALUE_DEFAULT, 0),
            'looking_away' => new external_value(PARAM_INT, 'Count of times looking away', VALUE_DEFAULT, 0),
            'severity' => new external_value(PARAM_ALPHA, 'Alert severity level', VALUE_DEFAULT, 'warning'),
        ]);
    }

    public static function log_eyetrack_alert($eyeNotVisible = 0, $lookingAway = 0, $severity = 'warning') {
        global $DB, $USER;
        $params = self::validate_parameters(self::log_eyetrack_alert_parameters(),
            ['eye_not_visible' => $eyeNotVisible, 'looking_away' => $lookingAway, 'severity' => $severity]);

        $record = new stdClass();
        $record->eye_not_visible = $params['eye_not_visible'];
        $record->looking_away = $params['looking_away'];
        $record->alert_severity = $params['severity'];
        $record->timedetected = time();

        $id = $DB->insert_record('quizaccess_proctoring_eyetrack_alerts', $record);

        return ['success' => true, 'id' => $id];
    }

    public static function log_eyetrack_alert_returns() {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Operation success'),
            'id' => new external_value(PARAM_INT, 'Alert record ID'),
        ]);
     ...
     */
    public static function log_reverification_parameters() {
        return new external_function_parameters([
            'reverification_number' => new external_value(PARAM_INT, 'Sequential re-verification count'),
            'status' => new external_value(PARAM_ALPHA, 'Verification status', VALUE_DEFAULT, 'pending'),
            'face_match_score' => new external_value(PARAM_INT, 'Face match score', VALUE_DEFAULT, 0),
            'quiz_paused_duration' => new external_value(PARAM_INT, 'Quiz pause duration in seconds', VALUE_DEFAULT, 0),
        ]);
    }

    public static function log_reverification($reverificationNumber, $status = 'pending', $faceMatchScore = 0, $quizPausedDuration = 0) {
        global $DB, $USER;
        $params = self::validate_parameters(self::log_reverification_parameters(),
            ['reverification_number' => $reverificationNumber, 'status' => $status,
             'face_match_score' => $faceMatchScore, 'quiz_paused_duration' => $quizPausedDuration]);

        $record = new stdClass();
        $record->reverification_number = $params['reverification_number'];
        $record->verification_status = $params['status'];
        $record->face_match_score = $params['face_match_score'];
        $record->quiz_paused_duration = $params['quiz_paused_duration'];
        $record->timecreated = time();
        $record->timecompleted = ($params['status'] !== 'pending') ? time() : 0;

        $id = $DB->insert_record('quizaccess_proctoring_reverification', $record);

        return ['success' => true, 'id' => $id];
    }

    public static function log_reverification_returns() {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Operation success'),
            'id' => new external_value(PARAM_INT, 'Re-verification record ID'),
        ]);
    }
     ...
     */
    public static function update_reverification_parameters() {
        return new external_function_parameters([
            'reverification_number' => new external_value(PARAM_INT, 'Re-verification sequence number'),
            'status' => new external_value(PARAM_ALPHA, 'New status'),
            'face_match_score' => new external_value(PARAM_INT, 'Face match score', VALUE_DEFAULT, 0),
            'quiz_paused_duration' => new external_value(PARAM_INT, 'Pause duration', VALUE_DEFAULT, 0),
        ]);
    }

    public static function update_reverification($reverificationNumber, $status, $faceMatchScore = 0, $quizPausedDuration = 0) {
        global $DB;
        $params = self::validate_parameters(self::update_reverification_parameters(),
            ['reverification_number' => $reverificationNumber, 'status' => $status,
             'face_match_score' => $faceMatchScore, 'quiz_paused_duration' => $quizPausedDuration]);
        ...
        $success = $DB->update_record('quizaccess_proctoring_reverification', $record);

        return ['success' => $success];
    }

    public static function update_reverification_returns() {
        return new external_single_structure([
     ...
     */
    public static function generate_analytics_parameters() {
        return new external_function_parameters([
            'total_captures' => new external_value(PARAM_INT, 'Total webcam captures', VALUE_DEFAULT, 0),
            'multiuser_alerts' => new external_value(PARAM_INT, 'Multi-user alert count', VALUE_DEFAULT, 0),
            'eyetrack_alerts' => new external_value(PARAM_INT, 'Eye tracking alert count', VALUE_DEFAULT, 0),
            'reverifications_triggered' => new external_value(PARAM_INT, 'Re-verification count', VALUE_DEFAULT, 0),
            'total_pause_time' => new external_value(PARAM_INT, 'Total pause time in seconds', VALUE_DEFAULT, 0),
            'timestamp' => new external_value(PARAM_INT, 'Analytics timestamp', VALUE_DEFAULT, 0),
        ]);
    }

    public static function generate_analytics($totalCaptures = 0, $multiuserAlerts = 0, $eyetrackAlerts = 0,
                                              $reverificationsTriggered = 0, $totalPauseTime = 0, $timestamp = 0) {
        global $DB, $USER;
        $params = self::validate_parameters(self::generate_analytics_parameters(),
            ['total_captures' => $totalCaptures, 'multiuser_alerts' => $multiuserAlerts,
             'eyetrack_alerts' => $eyetrackAlerts, 'reverifications_triggered' => $reverificationsTriggered,
             'total_pause_time' => $totalPauseTime, 'timestamp' => $timestamp]);

        // Calculate risk score
        $riskScore = quizaccess_proctoring_calculate_risk_score($params['multiuser_alerts'],
                                                                $params['eyetrack_alerts'], 0, 70);
        ...
        $record = new stdClass();
        $record->total_captures = $params['total_captures'];
        $record->multiuser_alerts = $params['multiuser_alerts'];
        $record->eyetrack_alerts = $params['eyetrack_alerts'];
        $record->reverifications_triggered = $params['reverifications_triggered'];
        $record->total_pause_time = $params['total_pause_time'];
        $record->risk_score = $riskScore;
        $record->risk_level = $riskLevel;
        $record->compliance_status = $complianceStatus;
        $record->timecreated = $params['timestamp'] ?: time();

        $id = $DB->insert_record('quizaccess_proctoring_analytics', $record);
    ...
    public static function generate_analytics_returns() {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Operation success'),
            'id' => new external_value(PARAM_INT, 'Analytics record ID'),
            'risk_score' => new external_value(PARAM_INT, 'Calculated risk score'),
            'risk_level' => new external_value(PARAM_ALPHA, 'Risk level'),
            'compliance_status' => new external_value(PARAM_ALPHA, 'Compliance status'),
        ]);
    }
    ...
    public static function get_analytics($reportid) {
        global $DB;
        $params = self::validate_parameters(self::get_analytics_parameters(),
            ['reportid' => $reportid]);

        $record = $DB->get_record('quizaccess_proctoring_analytics', ['reportid' => $params['reportid']]);
    ...
    public static function get_analytics_returns() {
        return new external_single_structure([
            ...
            'analytics' => new external_multiple_structure(
                new external_value(PARAM_RAW, 'Analytics field'),
                'Analytics data',
                VALUE_OPTIONAL
            ),
        ]);
    ...
    /**
     * Parameters for extend_attempt_time.
     *
     * @return external_function_parameters
     */
    public static function extend_attempt_time_parameters() {
        return new external_function_parameters([
            'attemptid' => new external_value(PARAM_INT, 'Quiz attempt id'),
            'cmid' => new external_value(PARAM_INT, 'Course module id'),
            'pauseseconds' => new external_value(PARAM_INT, 'Seconds to add back to the attempt'),
        ]);
    }

    /**
     * Extend the running quiz attempt's deadline by $pauseseconds.
     *
     * Used when "Pause quiz timer during verification" is enabled on the quiz:
     * after a face check succeeds we add the modal-open duration back to the
     * student's remaining time by pushing quiz_attempts.timestart forward.
     *
     * The call is bounded (1..600s) and only applies when:
     *   - the caller actually owns the attempt;
     *   - the attempt is still in progress; and
     *   - the proctoring rule for the quiz has pausequiztime enabled.
     *
     * @param int $attemptid
     * @param int $cmid
     * @param int $pauseseconds
     * @return array {extended:int, message:string}
     */
    public static function extend_attempt_time($attemptid, $cmid, $pauseseconds) {
        global $DB, $USER;

        self::validate_parameters(self::extend_attempt_time_parameters(), [
            'attemptid' => $attemptid,
            'cmid' => $cmid,
            'pauseseconds' => $pauseseconds,
        ]);

        // Cap pause: > 10 minutes for a single verification is implausible and
        // almost certainly indicates a stuck client or tampering.
        if ($pauseseconds < 1 || $pauseseconds > 600) {
            return ['extended' => 0, 'message' => 'out of range'];
        }

        $attempt = $DB->get_record('quiz_attempts', ['id' => $attemptid], '*', MUST_EXIST);
        if ((int) $attempt->userid !== (int) $USER->id) {
            throw new moodle_exception('accessdenied', 'quizaccess_proctoring');
        }
        if ($attempt->state !== 'inprogress') {
            return ['extended' => 0, 'message' => 'not in progress'];
        }

        // Verify the pause feature is actually enabled for this quiz so we
        // can't be tricked into extending a quiz that didn't opt in.
        $proctoringsettings = $DB->get_record('quizaccess_proctoring', ['quizid' => $attempt->quiz]);
        if (!$proctoringsettings || empty($proctoringsettings->pausequiztime)) {
            return ['extended' => 0, 'message' => 'pause not enabled'];
        }

        $attempt->timestart = (int) $attempt->timestart + (int) $pauseseconds;
        $DB->update_record('quiz_attempts', $attempt);

        return ['extended' => (int) $pauseseconds, 'message' => 'ok'];
    }

    /**
     * Return shape for extend_attempt_time.
     *
     * @return external_single_structure
     */
    public static function extend_attempt_time_returns() {
        return new external_single_structure([
            'extended' => new external_value(PARAM_INT, 'Seconds applied to the attempt'),
