    ...
    function($, Ajax, Notification, Str) {
        const loadStrings = async function() {
            const stringkeys = [
                ...
                {key: 'serviceunavailable', component: 'quizaccess_proctoring'},
            ];
            try {
                    ...
                    Ajax.call([request])[0].done(function(res) {
                        if (res.warnings.length < 1) {
                            ...
                            if (status === 'success') {
                                ...
                                $("#video").css("border", "10px solid orange");
                                $("#face_validation_result").html(`<span style="color: orange">${strings.serviceunavailable}</span>`);
                            } else {
                                $("#video").css("border", "10px solid red");
