...
if (
    ...
    if ($studentid != null && $cmid != null && $courseid != null && $reportid != null) {
        ...
        // Aggregate session summary (filled as we walk the per-frame rows).
        $summary = [
            'frames_total' => 0,
            'analyzed_total' => 0,         // awsflag == 2 (face match actually ran)
            'face_match_success' => 0,
            'face_match_failed' => 0,
            'score_sum' => 0,
            'behavior_total' => 0,         // frames that have a behavior_result
            'no_face' => 0,
            'multiple_faces' => 0,
            'unusual_head' => 0,
            'unusual_gaze' => 0,
        ...
        foreach ($sqlexecuted as $info) {
                ...
                if ($summary['first_time'] === null || $info->timemodified < $summary['first_time']) {
                    $summary['first_time'] = (int) $info->timemodified;
                }
                if ($summary['last_time'] === null || $info->timemodified > $summary['last_time']) {
                    $summary['last_time'] = (int) $info->timemodified;
                }

                // Face match column. awsflag==2 means the comparison actually
                // ran; anything else means it never produced a score (queued,
                ...
                // glance even when the face matched.
                $row['risk_level'] = '';
                $row['risk_indicators'] = '';
                $row['gaze_offset'] = null;
                if (!empty($info->behavior_result)) {
                    $behavior = json_decode($info->behavior_result);
                    if ($behavior && isset($behavior->risk_level)) {
                        $summary['behavior_total']++;
                        $row['risk_level'] = (string) $behavior->risk_level;
                        if (isset($behavior->suspicious_indicators)) {
                            $row['risk_indicators'] = implode(', ', (array) $behavior->suspicious_indicators);
                        }
                        if (!empty($behavior->no_face_detected)) {
                            $summary['no_face']++;
                        }
                        if (!empty($behavior->multiple_faces_detected)) {
                            $summary['multiple_faces']++;
                        }
                        if (!empty($behavior->unusual_head_pose)) {
                            $summary['unusual_head']++;
                        }
                        if (!empty($behavior->unusual_eye_gaze)) {
                            $summary['unusual_gaze']++;
                        }
                        if (isset($behavior->gaze_offset) && is_numeric($behavior->gaze_offset)) {
                            $row['gaze_offset'] = (float) $behavior->gaze_offset;
                            $summary['gaze_offset_sum'] += abs((float) $behavior->gaze_offset);
                            $summary['gaze_offset_count']++;
                        }
        ...
        $summarycontext = [
            ...
            'analyzed_total' => $summary['analyzed_total'],
            'face_match_success' => $summary['face_match_success'],
            'face_match_failed' => $summary['face_match_failed'],
            'success_rate' => $summary['analyzed_total']
                ? round(100 * $summary['face_match_success'] / $summary['analyzed_total'])
                : 0,
            'avg_score' => $summary['analyzed_total']
                ? round($summary['score_sum'] / $summary['analyzed_total'])
                : 0,
            ...
            'duration_minutes' => ($summary['first_time'] && $summary['last_time'])
                ? max(0, (int) round(($summary['last_time'] - $summary['first_time']) / 60))
                : 0,
            'start_time' => $summary['first_time']
                ? userdate($summary['first_time'], get_string('strftimedatetimeshort', 'langconfig'))
                : '',
        ...
        if ($summarycontext['risk_high'] > 0) {
            $summarycontext['overall_risk'] = 'high';
        } else if ($summarycontext['risk_medium'] >= 3) {
            $summarycontext['overall_risk'] = 'medium';
        } else if ($summary['behavior_total'] > 0) {
            $summarycontext['overall_risk'] = 'low';
        } else {
            $summarycontext['overall_risk'] = '';
        }
        ...
        $templatecontext = (object)[
            ...
            'hasframes' => $summary['frames_total'] > 0,
        ];
