 ...
 * @author      Brain station 23 ltd <brainstation-23.com>; Evanka Fauzya
 * @copyright   2020 Brain station 23 ltd, 2026 Evanka Fauzya
...
function xmldb_quizaccess_proctoring_upgrade($oldversion) {
    ...
    if ($oldversion < 2026052301) {
        // Add behavior_result column to quizaccess_proctoring_logs for storing
        // the AI backend's /detect/behavior JSON output (suspicious-behavior
        // analysis run by the admin Analyze flow).
        $table = new xmldb_table('quizaccess_proctoring_logs');
        $field = new xmldb_field('behavior_result', XMLDB_TYPE_TEXT, null, null, null, null, null, 'deletionprogress');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_plugin_savepoint(true, 2026052301, 'quizaccess', 'proctoring');
    }

    if ($oldversion < 2026052303) {
        // Add per-quiz re-verification settings: how often to re-verify
        // (0 = disabled) and whether to pause the quiz timer during it.
        $table = new xmldb_table('quizaccess_proctoring');

        $field1 = new xmldb_field('reverifyinterval', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '120',
            'proctoringrequired');
        if (!$dbman->field_exists($table, $field1)) {
            $dbman->add_field($table, $field1);
        }

        $field2 = new xmldb_field('pausequiztime', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0',
            'reverifyinterval');
        if (!$dbman->field_exists($table, $field2)) {
            $dbman->add_field($table, $field2);
        }
